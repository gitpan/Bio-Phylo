...
The version you are reading now is: $Revision: 4265 $
...
All three packages now have their own IO architecture. However,
the BioPerl *IO classes now can take factories as arguments, 
allowing them (in theory) to instantiate objects from L<Bio::Phylo>
or L<Bio::NEXUS> while using its internal parsers. 

Work is now well under way to make
