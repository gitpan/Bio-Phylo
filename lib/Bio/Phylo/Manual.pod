...
The C<-e> switch is used to evaluate the subsequent expression. We
parse a string, C<((A,B),C);>, of format C<newick>. The parser
returns a L<Bio::Phylo::Forest> object (i.e. a set of trees, in this case
a set of one). From this set we retrieve the first (and only) tree, and
calculate Colless' imbalance, which returns a number, which we print to standard
out. This would print "1", because the tree is a ladder, and therefore completely
unbalanced.
...
the front door for the parsers. In the solution the
L<Bio::Phylo::Parsers::Newick> parser turns a tree description into a
L<Bio::Phylo::Forest> object. 
...
I know what version you're using:
 ...
 $Id: Manual.pod 3409 2007-03-27 21:27:44Z rvosa $
